...
- **Liveness gap between `complete()==true` break and `ACTIVE→IDLE` CAS**: a write-fail during this window called `toggle_resync()` which `launch()`-EARLY_EXITs on `ACTIVE` state; dirty bits were then stranded with no resync running. Added `on_idle_dirty` callback to `_start()`/`launch()`; after the `ACTIVE→IDLE` CAS, calls `toggle_resync(true)` if `dirty_pages() > 0`. Self-join prevented by detecting the resync thread calling `launch()` on itself (detach instead of join).
