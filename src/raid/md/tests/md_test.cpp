...
constexpr uint64_t k_leg_size = 128UL * Mi;
...
TEST(MdDiscovery, ScrubsMdSbAfterWrap) {
    ...
    EXPECT_EQ(leaf->data()[4 * Ki], 0xfc); // 0xa92b4efc LE first byte
    {
        auto md = std::make_shared< ublkpp::md::MdDisk >(leaf, ublkpp::md::test::default_md_uuid());
        ASSERT_NE(md, nullptr);
        md->prepare(nullptr, 0); // stamp own SB and scrub md SB
    }
...
TEST(MdDiscovery, ReattachReadsOwnSbWithoutMdSb) {
    auto leaf = make_clean_md_leg("leg0", /*dev_role=*/2);
    {
        ...
        md1->prepare(nullptr, 0); // stamp own SB and scrub md SB
    }
    // First prepare() stamped own SB and scrubbed md SB. Second wrap must succeed
    // by reading our own SB at byte 0 (md SB at byte 4096 is now zeroed).
...
TEST(MdTranslation, HeadZoneShiftsByOnePage) {
    auto leaf = make_clean_md_leg("leg0", 0);
    auto md = std::make_shared< ublkpp::md::MdDisk >(leaf, ublkpp::md::test::default_md_uuid());
    md->prepare(nullptr, 0); // stamp own SB so byte-0 check below is valid
...
TEST(MdProbe, PlainDeviceReturnsMdNone) {
    // BufferedDisk with no SB staged (0xa5 fill) - neither magic matches.
    auto d = std::make_shared< BufferedDisk >(k_leg_size, "plain");
    EXPECT_EQ(ublkpp::md::probe(d), ublkpp::md::disk_type::md_none);
}

TEST(MdProbe, CleanMdSbReturnsMdNative) {
    // md SB at byte 4096, resync_offset == MaxSector.
    auto d = make_clean_md_leg("leg0", 0);
    EXPECT_EQ(ublkpp::md::probe(d), ublkpp::md::disk_type::md_native);
}

TEST(MdProbe, DirtyMdSbReturnsMdDirty) {
    auto d = std::make_shared< BufferedDisk >(k_leg_size, "dirty");
    MdSbBuilder b{};
    b.resync_offset = 0; // not clean
    stage_md_sb(*d, b);
    EXPECT_EQ(ublkpp::md::probe(d), ublkpp::md::disk_type::md_dirty);
}

TEST(MdProbe, NoWriteBeforePrepare) {
    // Construction must not modify any device; stamp is deferred to prepare().
    auto d = make_clean_md_leg("leg0", 0);
    auto const md_magic_byte = d->data()[4 * Ki]; // md magic first byte
    { std::make_shared< ublkpp::md::MdDisk >(d, ublkpp::md::test::default_md_uuid()); }
    // md SB at byte 4096 must be untouched; own SB at byte 0 must not be written.
    EXPECT_EQ(d->data()[4 * Ki], md_magic_byte);
    EXPECT_NE(std::memcmp(d->data(), ublkpp::md::k_magic, ublkpp::md::k_magic_size), 0);
}

TEST(MdProbe, StampedLegReturnsMdStamped) {
    // After prepare(), MdDisk stamps its SB at byte 0 on each leg.
    auto d0 = make_clean_md1_leg("leg0", 0);
    auto d1 = make_clean_md1_leg("leg1", 1);
    auto const& uuid = ublkpp::md::test::default_md_uuid();
    auto disk = ublkpp::md::make_md_raid1_disk(uuid, {d0, d1});
    disk->prepare(nullptr, 0); // cascades to both MdDisk leaves, triggering stamp
    // d0 is still valid (shared_ptr); probe reads byte 0 which now has MdDisk SB.
    EXPECT_EQ(ublkpp::md::probe(d0), ublkpp::md::disk_type::md_stamped);
}
