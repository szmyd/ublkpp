...
// Classify the md 1.2 SB at `sb` (a k_page_size buffer from byte k_md_sb_offset).
disk_type classify_md_sb(uint8_t const* sb) noexcept {
    if (le32_at(sb, k_md_off_magic) != k_md_magic) return disk_type::md_none;
    auto const feature_map = le32_at(sb, k_md_off_feature_map);
    if ((feature_map & (k_md_feature_reshape_active | k_md_feature_recovery_offset)) != 0 ||
        le64_at(sb, k_md_off_resync_offset) != k_md_sector_clean)
        return disk_type::md_dirty;
    return disk_type::md_native;
}
...
DiscoveredTopology discovered_topo_from_md(ublk_disk& leaf) {
    ...
    switch (classify_md_sb(sb)) {
    case disk_type::md_none:
        throw std::runtime_error("MdDisk: leaf has no md superblock magic at byte 4096 (not an md 1.2 array)");
    case disk_type::md_dirty:
        throw std::runtime_error("MdDisk: md array is not clean (resync/reshape/recovery active); cannot import");
    case disk_type::md_stamped:
    case disk_type::md_native:
        break;
    }
    ...
    RLOGI("MdDisk: discovered md-raid {} topology on {}: set_uuid={}, raid_disks={}, dev_role={}, "
...
void MdDisk::__probe_and_init(boost::uuids::uuid const& expected_uuid) {
    ...
    if (std::memcmp(mdsb->header.magic, k_magic, k_magic_size) == 0) {
        ...
        RLOGD("MdDisk: re-attached existing wrapper on {}: set_uuid={}, dev_role={}, data_offset={}MiB", _leaf->id(),
        ...
        // Writes are deferred to the first prepare() call so all legs are validated before
        // any device is modified.
        _stamp_pending.store(true, std::memory_order_relaxed);
    }
...
void MdDisk::__stamp() {
    AlignedBuf buf(_leaf->block_size(), k_page_size);
    auto* mdsb = static_cast< SuperBlock* >(buf.ptr);
    ...
    if (auto w = leaf_pwrite(*_leaf, 0, buf.ptr, k_page_size); !w)
        throw std::runtime_error(
            fmt::format("MdDisk: failed to write own SB on {}: {}", _leaf->id(), w.error().message()));
    std::memset(buf.ptr, 0, k_page_size);
    if (auto w = leaf_pwrite(*_leaf, k_md_sb_offset, buf.ptr, k_page_size); !w)
        throw std::runtime_error(
            fmt::format("MdDisk: failed to scrub md SB on {}: {}", _leaf->id(), w.error().message()));
    RLOGI("MdDisk: stamped wrapper SB on {}; md superblock at byte {} scrubbed", _leaf->id(), k_md_sb_offset);
}
...
ublk_disk::prepare_result MdDisk::prepare(ublksrv_queue const* q, int const iouring_device_start) {
    if (_stamp_pending.exchange(false, std::memory_order_acq_rel)) __stamp();
...
disk_task< int > MdDisk::async_iov(ublksrv_queue const* q, ublk_io_data const* data, iovec* iovecs, uint32_t nr_vecs,
    ...
    if (addr + len <= _data_band_threshold) {
        ...
        RLOGE("MdDisk: I/O straddles head-zone boundary [addr={:#x}, len={:#x}, threshold={:#x}]", addr, len,
...
io_result MdDisk::sync_iov(uint8_t op, iovec* iovecs, uint32_t nr_vecs, off_t addr) noexcept {
    ...
    if (uaddr + len <= _data_band_threshold) {
        ...
        RLOGE("MdDisk: sync_iov straddles head-zone boundary [addr={:#x}, len={:#x}, threshold={:#x}]", uaddr, len,
              _data_band_threshold);
...
disk_handle make_md_raid10_disk(boost::uuids::uuid const& uuid, std::vector< disk_handle >&& legs,
    ...
    // Collect pairs in ascending pair-K order so md's chunk-to-pair mapping
    // (kernel uses chunk_index % (raid_disks / 2)) is preserved byte-identical.
    std::vector< disk_handle > ordered;
    ordered.reserve(num_pairs * 2);
    for (uint16_t k = 0; k < num_pairs; ++k) {
        ...
        ordered.push_back(std::move(it->second.first));
        ordered.push_back(std::move(it->second.second));
    }

    return make_raid10_disk(uuid, ref.md_chunk_size, std::move(ordered), parent_id);
}

disk_type probe(disk_handle const& leaf) {
    if (!leaf || leaf->is_missing()) throw std::invalid_argument("md::probe: leaf is null or missing");

    // Check byte 0 for MdDisk SB magic (reattach path).
    AlignedBuf buf0(leaf->block_size(), k_page_size);
    if (auto r = leaf_pread(*leaf, 0, buf0.ptr, k_page_size); !r)
        throw std::runtime_error(fmt::format("md::probe: read(0) on {}: {}", leaf->id(), r.error().message()));
    if (std::memcmp(buf0.ptr, k_magic, k_magic_size) == 0) return disk_type::md_stamped;

    // Check byte 4096 for md 1.2 SB magic (fresh import path).
    AlignedBuf buf4(leaf->block_size(), k_page_size);
    if (auto r = leaf_pread(*leaf, k_md_sb_offset, buf4.ptr, k_page_size); !r)
        throw std::runtime_error(
            fmt::format("md::probe: read({}) on {}: {}", k_md_sb_offset, leaf->id(), r.error().message()));
    return classify_md_sb(static_cast< uint8_t const* >(buf4.ptr));
}
