...
class Raid1ResyncTask {
    ...
    void __pause();
    inline void __resume() {
        auto cur_state = static_cast< uint8_t >(resync_state::PAUSE);
        _resync_state.compare_exchange_strong(cur_state, static_cast< uint8_t >(resync_state::ACTIVE));
    }
    ...
    resync_state __yield(std::chrono::microseconds const yield_for, std::chrono::microseconds const spin_time);
