...
// Drive resync ticks until bytes_to_sync reaches 0 AND no device is SYNCING, or timeout elapses.
// bytes_to_sync drains before __finish_session() restores the route to EITHER; checking both
// avoids a race where we observe bytes_to_sync==0 but device_b still shows SYNCING.
static void wait_for_resync(ublkpp::raid1::Raid1Disk* raid, ublkpp::MockUblksrv& mock, int timeout_ms = 5000) {
...
// After a degrading write, enabling resync allows the tick-based task to copy the dirty region
...
TEST_F(AsyncRaid1Fixture, ResyncReadFailurePreservesDirty) {
    ...
    // Resync reads use async_iov → sync_iov; fail all reads from disk_a at data addresses.
    ...
    raid->toggle_resync(true);
    // Drive several ticks: first tick probes disk_b; subsequent ticks attempt reads on disk_a
    // (which fail), leaving the dirty region intact.
    for (int i = 0; i < 5; ++i)
        mock->resync_tick();
...
TEST_F(AsyncRaid1Fixture, ResyncWriteFailurePreservesDirty) {
    ...
    // Resync writes use async_iov → sync_iov; fail all writes to disk_b at data addresses.
    ...
    raid->toggle_resync(true);
    // Drive several ticks: first tick probes disk_b (READ succeeds); subsequent ticks attempt
    // the data write (fails), leaving the dirty region intact.
    for (int i = 0; i < 5; ++i)
        mock->resync_tick();
...
// hits the ACTIVE early-exit arm in __transition_to and returns immediately without re-starting
// the session. The original resync session proceeds to completion.
TEST_F(AsyncRaid1Fixture, ResyncLaunchWhileRunningIsNoop) {
    degrade_via_backup_fail(mock.get(), 0, 0, 32 * Ki / 512);
    ASSERT_GT(raid->replica_states().bytes_to_sync, 0u);

    raid->toggle_resync(true); // IDLE → ACTIVE
    EXPECT_TRUE(raid->is_active());

    // Each call hits the ACTIVE → early-exit arm inside launch(); state stays ACTIVE.
    for (int i = 0; i < 5; ++i)
        raid->toggle_resync(true);
    EXPECT_TRUE(raid->is_active());
...
// tracker. If the write overlaps a dirty chunk that resync is about to copy, the Phase 1
// conflict check causes resync to skip that chunk. Once the write completes (dequeue_write),
// resync retries and clears the region.
TEST_F(AsyncRaid1Fixture, ResyncBlockedByOutstandingWrites) {
    // Dirty region at LBA 0 via write failure.
    degrade_via_backup_fail(mock.get(), 0, 0, 32 * Ki / 512);
    ASSERT_EQ(raid->replica_states().bytes_to_sync, 32 * Ki);

    // Submit a write at the SAME dirty LBA (0) while the region is still dirty.
    // In degraded mode (disk_b unavail) only disk_a is written → 1 cqe_state.
    // enqueue_write(0, 32Ki) is called at write start (before inject_cqe).
    auto pending = mock->submit_io(3, UBLK_IO_OP_WRITE, 0, 32 * Ki / 512, nullptr);
    ASSERT_TRUE(pending);
    EXPECT_EQ(pending.value(), 1u); // degraded: only disk_a

    raid->toggle_resync(true);

    // First tick: disk_b is unavail → probe (clears unavail) → returns without copying.
    mock->resync_tick();
    // Second tick: cursor finds LBA 0 dirty; Phase 1 conflict (write in-flight) → skip.
    mock->resync_tick();
    EXPECT_GT(raid->replica_states().bytes_to_sync, 0u); // LBA 0 still dirty

    // Complete the write: disk_a ok, disk_b was skipped → dirty_region(0) + dequeue_write(0).
    auto comp = mock->inject_cqe(3, 32 * Ki);
    ASSERT_EQ(comp.size(), 1u);

    // Now resync can proceed: no conflicting write in-flight → copies LBA 0.
    wait_for_resync(raid.get(), *mock);
    EXPECT_EQ(raid->replica_states().bytes_to_sync, 0u);
    EXPECT_EQ(raid->replica_states().device_b, ublkpp::raid1::replica_state::CLEAN);
}
