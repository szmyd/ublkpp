...
// Test the critical race: write failure during active resync
// This verifies that bitmap is dirtied BEFORE counter is decremented
TEST(Raid1, WriteFailureDuringActiveResync) {
    auto device_a = CREATE_DISK_A(TestParams{.capacity = Gi});
    auto device_b = CREATE_DISK_B(TestParams{.capacity = Gi});
    auto raid_device = ublkpp::Raid1Disk(boost::uuids::string_generator()(test_uuid), device_a, device_b);
    ...
    // Step 1: Create an initial dirty region
    {
        EXPECT_CALL(*device_a, async_iov(_, _, _, _, _, _))
            .Times(1)
            .WillOnce([](ublksrv_queue const*, ublk_io_data const*, ublkpp::sub_cmd_t, iovec*, uint32_t, uint64_t) {
    ...
    // Step 2: Set up flexible expectations for resync
    EXPECT_CALL(*device_a, sync_iov(::testing::_, _, _, _))
        ...
        .WillRepeatedly([](uint8_t op, iovec* iovecs, uint32_t nr_vecs, off_t) -> io_result {
            ...
            if (UBLK_IO_OP_READ == op && nullptr != iovecs->iov_base) {
                memset(iovecs->iov_base, 0xBB, iovecs->iov_len);
            }
            return ublkpp::__iovec_len(iovecs, iovecs + nr_vecs);
        });
    ...
    // Step 4: Issue a write that will fail on device B while resync is active
    // This tests the critical path: __handle_async_retry must dirty bitmap BEFORE DEQUEUE
    {
        ublkpp::sub_cmd_t primary_sub, replica_sub;
        bool got_replica = false;

        EXPECT_CALL(*device_a, async_iov(_, _, _, _, _, _))
            .Times(1)
            .WillOnce([&primary_sub](ublksrv_queue const*, ublk_io_data const*, ublkpp::sub_cmd_t sub_cmd, iovec*,
                                     uint32_t, uint64_t) {
                primary_sub = sub_cmd;
                return 1;
            });
        ...
        EXPECT_CALL(*device_b, async_iov(_, _, _, _, _, _))
            .Times(1)
            .WillOnce([&replica_sub, &got_replica](ublksrv_queue const*, ublk_io_data const*,
                                                    ublkpp::sub_cmd_t sub_cmd, iovec*, uint32_t, uint64_t) {
                replica_sub = sub_cmd;
                got_replica = true;
                return 1; // Success initially
            });

        auto ublk_data = make_io_data(UBLK_IO_OP_WRITE);
        auto res = raid_device.handle_rw(nullptr, &ublk_data, 0b10, nullptr, 32 * Ki, 256 * Ki);
        ASSERT_TRUE(res);
        EXPECT_EQ(2, res.value()); // Both devices get writes

        // Complete primary successfully
        raid_device.on_io_complete(&ublk_data, primary_sub, 0);

        // Complete replica with FAILURE (async failure after queuing)
        // This triggers __handle_async_retry path
        ASSERT_TRUE(got_replica);
        raid_device.on_io_complete(&ublk_data, replica_sub, -EIO);

        // Now the RETRIED internal response needs to be queued
        auto retry_sub = ublkpp::set_flags(replica_sub, ublkpp::sub_cmd_flags::RETRIED);
        auto retry_res = raid_device.handle_rw(nullptr, &ublk_data, retry_sub, nullptr, 32 * Ki, 256 * Ki);
        ASSERT_TRUE(retry_res);
        EXPECT_EQ(0, retry_res.value()); // Internal response, no actual I/O

        remove_io_data(ublk_data);
    }

    // Step 5: Stop resync immediately to capture the state before it cleans the newly dirtied region
    // The point of this test is to verify the bitmap was dirtied BEFORE counter decremented,
    // not to let resync clean it again
    raid_device.toggle_resync(false);
    std::this_thread::sleep_for(10ms); // Give resync time to stop

    // Step 6: Verify the failed write region is still dirty
    // The bitmap should have been dirtied BEFORE counter was decremented
    // so resync shouldn't have incorrectly cleaned it
    auto cur_replica_state = raid_device.replica_states();

    // We should have at least the newly failed region (256Ki) still dirty
    // It's 32Ki in size (chunk-aligned)
    // The original 64Ki region may or may not be cleaned depending on timing,
    // but the 256Ki region should definitely still be dirty
    EXPECT_GE(cur_replica_state.bytes_to_sync, 32 * Ki) << "Failed write region should remain dirty";
