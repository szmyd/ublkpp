...
TEST(Raid1MemoryEstimation, SmallVolume) {
    ...
    uint64_t memory = ublkpp::raid1::estimate_device_overhead(volume_size);
...
TEST(Raid1MemoryEstimation, LinearScaling) {
    ...
    uint64_t mem_1tib = ublkpp::raid1::estimate_device_overhead(vol_1tib);
    uint64_t mem_10tib = ublkpp::raid1::estimate_device_overhead(vol_10tib);
