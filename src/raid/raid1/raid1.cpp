...
public:
    ...
    std::atomic< uint16_t > skipped_cmds;
};
...
io_result Raid1Disk::__replicate(sub_cmd_t sub_cmd, auto&& func, uint64_t addr, uint32_t len, ublksrv_queue const* q,
    ...
    if (IS_DEGRADED) {
        if (auto dirty_degrade = DIRTY_DEVICE->unavail.test(std::memory_order_acquire);
            dirty_degrade || _dirty_bitmap->is_dirty(addr, len)) {
            ...
            auto skipped_cmds = 0;
            if (dirty_degrade) skipped_cmds = DIRTY_DEVICE->skipped_cmds.fetch_add(1, std::memory_order_relaxed);
            if (!totally_aligned || (0 != (skipped_cmds % Ki)) ||
...
io_result Raid1Disk::handle_internal(ublksrv_queue const* q, ublk_io_data const* data, sub_cmd_t sub_cmd, iovec* iovecs,
    ...
    if (0 == res) {
        ...
        DIRTY_DEVICE->skipped_cmds.store(0, std::memory_order_relaxed);
