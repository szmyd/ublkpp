...
BACKING_FILE="$STATE_DIR/lun0.img"
...
EOF
    ...
    $SUDO tgtadm --control-port "$CTRL_PORT" --lld iscsi --op new --mode logicalunit --tid 1 --lun 1 -b "$BACKING_FILE"
...
stop() {
    ...
    if [[ -f "$PID_FILE" ]]; then
        $SUDO kill "$(cat "$PID_FILE")" 2>/dev/null || true
        # Give tgtd a moment to release the port before CTest tears down
        for _ in $(seq 1 20); do
            $SUDO kill -0 "$(cat "$PID_FILE")" 2>/dev/null || break
            sleep 0.1
        done
        rm -f "$PID_FILE" "$BACKING_FILE"
    fi
